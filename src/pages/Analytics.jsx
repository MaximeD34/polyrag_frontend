  ...
  function refreshCounts() {
    const checkAuth = async () => {
      try {
        ...
        const response1 = await axios.get(`${API_BASE_URL}/analytics`, {
          withCredentials: true,
          headers: {
            "X-CSRF-TOKEN": csrfToken,
          },
        ...
        if (error.response && error.response.status === 401) {
          ...
          if (csrfRefreshToken) {
            try {
              ...
              if (response2.ok) {
                const response3 = await axios.get(`${API_BASE_URL}/analytics`, {
                  withCredentials: true,
                  headers: {
                    "X-CSRF-TOKEN": Cookies.get("csrf_access_token"),
                  },
      ...
      <div className="max-w-screen-xl mx-auto px-4 md:px-8 pt-7 mb-12">
        ...
        <div className="items-start justify-between md:flex">
          <div className="max-w-lg">
            ...
            <p className="text-gray-600 mt-2">These are your analytics</p>
          </div>
        </div>
